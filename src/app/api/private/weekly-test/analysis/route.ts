...
import { AptitudeQuestion } from '@/backend/models';
import { weeklyTestAnswerZodSchema } from '@/backend/schema/weekly-test-answer';
import {
  SaveAnswerData,
  weekTestAnswerService,
} from '@/backend/services/weekly-test-answer.service';
...
export async function POST(request: Request) {
  try {
    ...
    const answerData: SaveAnswerData = {
      ...result.data,
      ...
      isCorrect: false, // Will be updated after checking the answer
    };

    // Get the question to check if the answer is correct
    const question = await AptitudeQuestion.findById(answerData.questionId);
    if (!question) {
      return sendResponse('Question not found', false, null, 404);
    }

    answerData.isCorrect = question.correctAnswer === answerData.selectedOption;
